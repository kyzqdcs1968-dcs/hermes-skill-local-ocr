...
| 预检（含脱敏参数） | `~/.hermes/skills/productivity/local-wechat-ocr/scripts/run_mac_ocr.sh --anonymize --dry-run "/绝对路径/材料.pdf"` |
...
3. 用户明确提到"脱敏"、"隐私"、"遮盖"、"保密"等需求时，追加 `--anonymize`；如用户指定模型，同时追加 `--anonymize-model <模型名>`。
